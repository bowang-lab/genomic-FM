...
DATASETS = ['real_clinvar_hyena-tiny']
...
def compute_mean_std_accuracy(vec_loader, n=10, runs=5):
    accuracies = []
    for _ in range(runs):
...
def eval(datasets,finetunes):
    for dataset in datasets:
        for finetune in finetunes:
          vec_loader = VectorLoader(dataset=dataset, checkpoint=CHECKPOINT_DIR[dataset] if finetune else None, random=True)
